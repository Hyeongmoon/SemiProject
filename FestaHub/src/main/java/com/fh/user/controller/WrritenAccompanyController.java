 ...
 */
@WebServlet("/wrac.fh")
public class WrritenAccompanyController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public WrritenAccompanyController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new UserService().selectWrAcCount(userNo);				//총 게시글 수
		currentPage 
		...
		ArrayList<Accompany> list = new UserService().selectWrAc(pi, userNo);
		
		
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		
		request.getRequestDispatcher("views/user/wrAccompany.jsp").forward(request, response);
	}
