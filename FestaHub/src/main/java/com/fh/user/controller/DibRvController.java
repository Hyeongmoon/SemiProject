 ...
 */
@WebServlet("/dibrv.fh")
public class DibRvController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DibRvController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new UserService().selectDibRvCount(userNo);				//총 게시글 수
		currentPage 
		...
		ArrayList<Review> list = new UserService().selectDibRv(pi, userNo);
		...
		request.getRequestDispatcher("views/user/dibReview.jsp").forward(request, response);
	}
