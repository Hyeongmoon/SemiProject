 ...
 */
@WebServlet("/rpfes.fh")
public class ReplyFestivalController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ReplyFestivalController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new UserService().selectRpFesCount(userNo);				//총 게시글 수
		currentPage 
		...
		ArrayList<FestivalComment> list = new UserService().selectRpFes(pi, userNo);
		...
		request.getRequestDispatcher("views/user/rpFestival.jsp").forward(request, response);
	}
