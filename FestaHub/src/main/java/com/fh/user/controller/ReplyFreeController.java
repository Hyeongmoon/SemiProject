 ...
 */
@WebServlet("/rpfree.fh")
public class ReplyFreeController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ReplyFreeController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new UserService().selectRpFreeCount(userNo);				//총 게시글 수
		currentPage 
		...
		ArrayList<FreeBoardComment> list = new UserService().selectRpFree(pi, userNo);
		...
		request.getRequestDispatcher("views/user/rpFree.jsp").forward(request, response);
	}
