...
/**
 * Servlet implementation class AdminAccompanyListController
 */
@WebServlet("/views/adminTool/adminTool.accompany")
public class AdminAccompanyListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdminAccompanyListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 데이터 갯수 가져오기
		listCount = new AccompanyService().selectListCount();
		...
		ArrayList<Accompany> list 
			= new AccompanyService().selectList(pi);
		
		// 응답데이터 보내기
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		
		request.getRequestDispatcher("adminAccompanyListView.jsp").forward(request, response);
