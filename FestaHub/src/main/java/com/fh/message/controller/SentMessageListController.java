...
/**
 * Servlet implementation class SentMessageListController
 */
@WebServlet("/list.sme")
public class SentMessageListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SentMessageListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 보낸 쪽지의 받았던 사람을 조회할 기준이될 로그인 유저의 userNo 뽑기
		...
		// 받은 쪽지 갯수 가져오기
		listCount = new MessageService().selectSentMessageListCount(userNo);
		...
		ArrayList<Message> list 
			= new MessageService().selecSentMessagetList(pi, userNo);
		
		// 응답데이터 보내기
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		
		// 보낸 메세지 리스트 조회 페이지에 포워딩 sentMessageListView.jsp
		request.getRequestDispatcher("views/message/sentMessageListView.jsp")
