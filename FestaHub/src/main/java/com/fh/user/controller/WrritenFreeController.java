 ...
 */
@WebServlet("/wrfree.fh")
public class WrritenFreeController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public WrritenFreeController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new UserService().selectWrFreeCount(userNo);				//총 게시글 수
		currentPage 
		...
		ArrayList<FreeBoard> list = new UserService().selectWrFree(pi, userNo);
		...
		request.getRequestDispatcher("views/user/wrFree.jsp").forward(request, response);
	}
