...
<body>
    ...
    <div class="container">
        <h2 class="title" style="font-size: 30px; color: #fff; background-color: #493375; padding: 15px 20px; border-radius: 10px; text-shadow: 2px 2px 5px rgba(0, 0, 0, 0.3); margin-bottom: 20px; border: 2px solid #6a5e9b; animation: pulse 1.5s infinite;">내가 쓴 게시물(공연후기)</h2>
        <br><hr>
        <div class="content-container">
            ...
            <div align="center" class="pagination">
                <% if(currentPage != 1) { %>
                    <button onclick="location.href = '<%= request.getContextPath() %>/wrrv.fh?currentPage=<%= currentPage - 1 %>';">&lt;</button>
                <% } %>

                <% for(int p = startPage; p <= endPage; p++) { %>
                    <% if(p != currentPage) { %>
                        <button onclick="location.href = '<%= request.getContextPath() %>/wrrv.fh?currentPage=<%= p %>';"><%= p %></button>
                    <% } else { %>
