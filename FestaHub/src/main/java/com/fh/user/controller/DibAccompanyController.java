 ...
 */
@WebServlet("/dibac.fh")
public class DibAccompanyController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DibAccompanyController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new UserService().selectDibAccompanyCount(userNo);				//총 게시글 수
		currentPage 
		...
		ArrayList<Accompany> list = new UserService().selectDibAccompany(pi, userNo);
		...
		request.getRequestDispatcher("views/user/dibAccompany.jsp").forward(request, response);
	}
