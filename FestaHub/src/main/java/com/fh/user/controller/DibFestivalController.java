 ...
 */
@WebServlet("/dibfes.fh")
public class DibFestivalController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DibFestivalController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new UserService().selectDibFesCount(userNo);				//총 게시글 수
		currentPage 
		...
		ArrayList<Festival> list = new UserService().selectDibFes(pi, userNo);
		...
		request.getRequestDispatcher("views/user/dibFestival.jsp").forward(request, response);
	}
